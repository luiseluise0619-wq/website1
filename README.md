...
npm run sim           # 에디터를 사람처럼 처음부터 끝까지 써 본다 (아래 참고)
```
...
### `npm run sim` — 에디터 시뮬레이터

```bash
npm run build && npm start                 # 다른 터미널
ADMIN_PASSWORD=... npm run sim
```

단위 테스트도 캔버스 점검도 '한 조각'만 봅니다. 정작 사람이 겪는 것은
**페이지를 만들고 → 블록을 넣고 → 글을 고치고 → 저장하고 → 발행해서 →
공개 화면에서 확인하는** 한 줄기입니다. 그 줄기 어딘가가 끊기면 조각 검사는
전부 통과해도 제품은 쓸 수 없습니다.

시뮬레이터는 그 줄기를 실제 브라우저로 그대로 밟습니다 — 새 페이지 만들기,
레이어 목록 확인, 섹션 추가, 블록 끌어다 놓기, 글·글자크기·글자색 고치기,
손잡이로 크기 조절, Ctrl+Z, 저장, 휴대폰 폭 미리보기, 발행, 공개 화면 확인.
만든 시험용 페이지는 끝나고 지웁니다.

실제로 이 시뮬레이터가 잡은 것: 레이어 목록이 0.8초마다 상태를 갈아 끼워
에디터 전체를 다시 그리게 만들던 문제. 화면이 계속 들썩여서 버튼 클릭이
아예 먹지 않았는데, 단위 테스트로는 보이지 않는 종류였습니다.
...
npm run sim           # 에디터 전 과정 시뮬레이션 (서버 + ADMIN_PASSWORD 필요)
```
